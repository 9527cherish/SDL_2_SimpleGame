...
ServerComonFunc::ServerComonFunc()
{
    logName = "../logs/server/logfile";
    logPath = "../logs/server";
    init_logger(logName);
}
...
ServerComonFunc &ServerComonFunc::getInstance()
{
    static ServerComonFunc instance;
    return instance;
}

void ServerComonFunc::init_logger(const std::string &log_name)
{
        // 自动创建日志目录
